import { downloadContentFromMessage, getContentType, jidNormalizedUser, proto, extractMessageContent, isJidGroup, isJidUser, } from "baileys";
...
const procMsg = async (originalMessage, socket, dStore) => {
    ...
    if (isQuoted) {
        ...
        if (quotedMsgId) {
            quotedContent = Object.entries(dStore.messages)
                .map(([a, i]) => i.find((b) => b.key.id === quotedMsgId))
    ...
    return {
        ...
        isBot,
        reply: async (text) => {
            return await socket.sendMessage(chatJid, { text }, {
                quoted: message,
